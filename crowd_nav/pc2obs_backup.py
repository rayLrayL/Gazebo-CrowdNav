...
def state_callback(data):
	...
	twist = Twist()
	twist.linear.x = data.pose[1].position.x
	twist.linear.y = data.pose[1].position.y
	twist.angular.z = euler[2]
	pub_state.publish(twist)
	time.sleep(0.01)
	robot_state = [twist.linear.x, twist.linear.y, twist.angular.z]
...
def pc2obs(voxel_size = 0.3, plot=False, ros=True):
	global points_raw, color_image_raw, bridge, currentStatus, handle_easy, pub
	#print(points_raw)
...
if __name__ == "__main__":
	pc2obs_init()
	while True:
		samples = pc2obs(voxel_size = 0.3, plot=False)
		# print(samples)
	f.close()
	exit()
